...
chaincodeInvoke() {
  ...
  infoln "Invoking on peer0.${ORG} on channel '$CHANNEL'..."
  local rc=1
  local COUNTER=1
  # continue to poll
  # we either get a successful response, or reach MAX RETRY
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    ...
    set -x
    peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer --tls --cafile $ORDERER_CA -C $CHANNEL -n ${CC_NAME} --peerAddresses localhost:7051 --tlsRootCertFiles $PEER0_UET_CA --peerAddresses localhost:9051 --tlsRootCertFiles $PEER0_HUS_CA -c ${CC_INVOKE_CONSTRUCTOR} >&log.txt
    res=$?
    { set +x; } 2>/dev/null
    let rc=$res
    COUNTER=$(expr $COUNTER + 1)
  ...
  cat log.txt
  if test $rc -eq 0; then
    successln "Invoke successful on peer0.${ORG} on channel '$CHANNEL'"
  else
...
chaincodeQuery() {
  ...
  infoln "Querying on peer0.${ORG} on channel '$CHANNEL'..."
  local rc=1
  local COUNTER=1
  # continue to poll
  # we either get a successful response, or reach MAX RETRY
  ...
  cat log.txt
  if test $rc -eq 0; then
    successln "Query successful on peer0.${ORG} on channel '$CHANNEL'"
  else
    fatalln "After $MAX_RETRY attempts, Query result on peer0.${ORG} is INVALID!"
  fi
}