...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating UET Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-uet.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating HUS Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-hus.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.uet uet
    peer_cert admin Admin@uet uet

    infoln "Creating HUS Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.hus hus
    peer_cert admin Admin@hus hus
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/uet/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    while [[ $rc -ne 0 && $COUNTER -lt $MAX_RETRY ]]; do
      sleep 1
      set -x
      fabric-ca-client getcainfo -u https://admin:adminpw@localhost:7054 --caname ca-uet --tls.certfiles "${PWD}/organizations/fabric-ca/uet/ca-cert.pem"
      res=$?
      { set +x; } 2>/dev/null
      rc=$res  # Update rc
      COUNTER=$((COUNTER + 1))
  ...
  infoln "Generating CCP files for UET and HUS"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm orderer.example peer0.uet peer0.hus
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/uet/msp organizations/fabric-ca/uet/tls-cert.pem organizations/fabric-ca/uet/ca-cert.pem organizations/fabric-ca/uet/IssuerPublicKey organizations/fabric-ca/uet/IssuerRevocationPublicKey organizations/fabric-ca/uet/fabric-ca-server.db'
