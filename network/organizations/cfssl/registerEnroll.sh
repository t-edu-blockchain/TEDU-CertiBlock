...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG/ca"
    mkdir -p "organizations/peerOrganizations/$ORG/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG/peers"
    mkdir -p "organizations/peerOrganizations/$ORG/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG-cert.pem"
...
function orderer_cert() {
    ...
    mkdir -p organizations/ordererOrganizations/example/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/example/orderers
    mkdir -p organizations/ordererOrganizations/example/tlsca

    CERT_DIR=organizations/ordererOrganizations/example
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/example/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/example/orderers/$USER/tls"
